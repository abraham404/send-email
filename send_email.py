...
def send_email(subject, body, to_emails, from_email, from_password, files=[]):
    ...
    try:
        ...
        )  # Puedes cambiarlo por tu servidor SMTP
        server.starttls()
                            ...
                            if year_link == year and (period_link >= period and period_link <= periodEnd):
                                ...
                                for email_destina in emails_destina:
                                    try:
                                        ...
                                        print(
                                            f"Serie de caracteres extraídos: {code_employee}"
                                        )
                ...
                if i % 2 != 0:
                    ...
                    if os.path.isfile(full_path):  # Verificar si es un archivo
                        with open(full_path, "r", encoding="utf-8") as f:
                            ...
                            #obtener el número del empleado de los archivos
                            num_employee = re.search(regex, name_ruta)
                            code_employee = num_employee.group(1)


                            #for para recorrer el arreglo de los números de empleados
                            for employee_num in send_email_employee_num:
                                if code_employee == employee_num:
...
def add_employee():
    ...
    print(
        f"Numero del empleado: {number_emplo}, nombre: {name_emplo}, Email: { email_emplo }, Empresa: {company_emplo}"
    )
...
def get_employee(id, company):
    ...
    try:
        ...
        if result:
            ...
            return (
                jsonify({"status": "error", "message": "Empleado no encontrado"}),
                404,
            )
